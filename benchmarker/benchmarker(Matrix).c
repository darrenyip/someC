...
int benchmark(void) {
    ...
    printf("\n -> 1) Number of vector elements : %lld", (long long)N * N);
    
    if( N < 1024) {
        printf("\n -> 2) Number of accuracy improvement loops: %e", (double)LOOP);
    } else {
        printf("\n -> 2) Number of accuracy improvement loops: 1");
    }
    
    printf("\n -> 3) Total computation time : %0.4e seconds", ElapsedTime);
    
    if (N >= 1024) {
        ElapsedTimeForNXN = ElapsedTime;
    } else {
        ElapsedTimeForNXN = ElapsedTime / ((double)LOOP);
    }
    
    printf("\n -> 4) Computation time for the complete NxN matrix multiplication : %0.4e seconds", ElapsedTimeForNXN);
    
    ElapsedTimePerVector = ElapsedTimeForNXN / N;
    
    printf("\n -> 5) Computation time per arithmetic operation : %0.4e seconds", ElapsedTimePerVector);
    
    printf("\n -> 6) Estimated OPs per second is: %0.3e  OPs per second\n", OPS = (OPS_PER_INSTR / ElapsedTimePerVector));
    printf("\n -> 6) Estimated number of clock cycles per OP is %0.2f CPU Clock Cycles per OP\n", (CPU_CLK) / (OPS));
